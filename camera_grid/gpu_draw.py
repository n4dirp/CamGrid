"""GPU drawing helpers for viewport camera grid overlay."""
...
void main() {
    ...
    // Using -0.5 to 0.5 centers the anti-aliasing exactly on the bounding
    // edge so the capsule accurately fits the provided width and height.
...
def _draw_text_with_shadow(font_id: int, text: str, x: float, y: float, color: tuple[float, ...], scale: float):
    if len(color) == 3:
        color = color + (1.0,)
...
def _draw_filled_rounded_rect(x, y, w, h, r, color):
    ...
    shader.bind()
    shader.uniform_float("ModelViewProjectionMatrix", gpu.matrix.get_projection_matrix())
...
def _draw_pill(x, y, w, h, color):
    """Draw a filled pill (capsule) shape using GPU shader."""
    if w <= 0 or h <= 0:
        return
    ...
    # Give the quad 2 pixels of padding to allow the shader room to
    # fully complete its anti-aliasing gradient before the geometry ends.
    pad = 2.0
...
def _draw_pill_border(x, y, w, h, color, line_width=1.0):
    """Draw a pill (capsule) border outline using GPU shader."""
    if w <= 0 or h <= 0:
        return
